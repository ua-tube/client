import { AppHead, HomeLayout, buttonVariants, CategoryPills, VideosList } from '@/components'
...
export const getServerSideProps: GetServerSideProps<{
	...
	try {
		...
		const { data: creator } = await CreatorService.getCreatorByNicknameOrUserId({ nickname })
		const { data: videos } = await LibraryService.getVideos({
			creatorId: creator.id,
			page: 1,
			perPage: 32,
			...(sortOptions && { sortOrder: sortOptions.sortOrder, sortBy: sortOptions.sortBy })
		})
					...
					<div
						...
						children={[
							...
							<Link
								...
								className={buttonVariants({ variant: value.key === 'videos' ? 'secondary' : 'outline' })}
							/>
						))}
					/>
					<CategoryPills
						...
						onChange={async () => push({ query: { sort: currOption?.sortBy, nickname: creator.nickname }, pathname })}
					/>
