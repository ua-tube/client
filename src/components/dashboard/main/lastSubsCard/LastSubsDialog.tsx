...
const LastSubscriptionsDialog: FC<ILastSubscriptionsDialogProps> = ({
	setOpen,
	open
}) => {
	return (
		<Dialog onOpenChange={setOpen} open={open}>
			<DialogContent className='sm:min-w-fit'>
				<DialogHeader>
					...
					<DialogDescription>
						Включає лише користувачів, які зробили свої підписки
						загальнодоступними за 90 днів
					</DialogDescription>
				</DialogHeader>
				<Table>
					<TableHeader>
					...
					<TableBody
						children={subscriptions.map((sub, index) => (
							<TableRow key={index}>
								...
								<TableCell>
									{new Date(
										new Date().getTimezoneOffset() - index
									).toLocaleString()}
								</TableCell>
