import { formatNumbers, formatTimeAgo, getChannelUrl, getUserInitials, getVideoUrl } from '@/utils'
				...
				<div className="flex flex-row items-center space-x-3 w-full md:w-auto">
					<Link href={getChannelUrl(video?.creator?.nickname)}>
						<Avatar>
							...
							<AvatarFallback children={getUserInitials(video?.creator?.displayName)} />
						</Avatar>
					</Link>
					<div className="flex flex-col">
						<Link href={getChannelUrl(video?.creator?.nickname)}>
							<h5 className="font-semibold" children={video?.creator?.displayName} />
						</Link>
						<p
