...
import { getChannelUrl, getVideoUrl, formatDuration, cn } from '@/utils'
...
const CurrentVideoPlaylist: FC<ICurrentVideoPlaylistProps> = ({ currVideoId, currList }) => {
	...
	return <Card className="w-full rounded-lg">
		<CardHeader>
			<CardTitle children={currList.name} />
			<CardDescription className="flex flex-wrap items-center gap-2 py-2">
				...
				<div
					className="bg-primary-foreground px-1.5 py-1 text-sm rounded-lg max-w-fit flex space-x-1 items-center"
					children={`${currVideoIndex}/${currList.videos?.length}`}
				/>
			</CardDescription>
		</CardHeader>
		<ScrollArea
			...
			children={
				currList.videos?.map((value, index) =>
					<div key={index} className={cn('flex flex-row items-center space-x-2 hover:bg-secondary px-4 py-3',value.id === currVideoId &&  'bg-secondary') }>
