...
const HistoryVideo: FC<IHistoryContentProps> = ({
	value,
	setShareVideo,
	onVideoDelete
}) => {
...
const HistoryContent: FC = () => {
	...
	const onVideoDelete = async (id: string) => {
		try {
			const updatedHistory = history.map(history => ({
				...history,
				videos: history.videos.filter(video => video._id !== id)
			}))
	...
	const onPauseOrResumeHistoryRecord = async () => {
		try {
			...
			toast.success(
				t(recordWatchHistoryEnabled ? 'pausedHistory' : 'allowedRecordHistory')
			)
		} catch (e) {
			toastError(e)
		}
							...
							<div key={index}>
								...
								<div
									...
									children={group.videos.map((value, key) => (
										<HistoryVideo
											{...{ value, key, onVideoDelete, setShareVideo }}
										/>
									))}
								/>
							</div>
			...
			<Card className='w-full lg:w-1/4 h-fit rounded-xl lg:sticky lg:top-20 bg-repeat-space bg-center bg-cover'>
				...
				<CardContent>
					<div className='space-y-2'>
						...
						>
							<DynamicIcon
								name={recordWatchHistoryEnabled ? 'pause' : 'play'}
							/>
							<span>
								{t(recordWatchHistoryEnabled ? 'stop' : 'reniew')}{' '}
								{t('historyView')}
							</span>
						</Button>
					</div>
